...
|---|---:|---:|---:|
| 长篇单章：明确必读指令下限 | 30,979 | 7,023 | 77.33% |
| 多线超长篇：含完整超长篇附加流程 | 30,979 | 9,610 | 68.98% |
| 短篇：上游仅入口 vs 新版入口与流程 | 10,422 | 7,023 | 32.61% |
| 长篇拆文：上游仅入口 vs 新版入口与流程 | 8,996 | 5,068 | 43.66% |
| 深读示范：含按需分析对照样例 | 8,996 | 7,077 | 21.33% |
| 审稿：上游仅入口 vs 新版入口与流程 | 11,334 | 3,063 | 72.98% |
| 冲突与看点审查：含中文场景参考 | 11,334 | 4,528 | 60.05% |

原始文件清单、SHA-256 和每个场景的统计边界见 [tokens.json](tokens.json)。

发现元数据：上游 13 个入口合计 1257 tokens；新版 7 个入口 313 tokens。仅含名称/描述；旧技能仍启用时，不能把这个差额当成真实节省。

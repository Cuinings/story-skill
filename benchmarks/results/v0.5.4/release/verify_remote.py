#!/usr/bin/env python3
"""Read-only v0.5.4 Release download and official isolated-install verification."""
...
def bind_commit_payload(commit, expected_files):
    """Bind reviewed payload bytes to immutable Git blobs, not just the checkout."""
    entries = subprocess.check_output(["git", "ls-tree", "-r", "-z", commit, "--", "skills"], cwd=ROOT)
    committed = {}
    for entry in entries.split(b"\0"):
        ...
        if mode not in (b"100644", b"100755") or kind != b"blob":
            raise ValueError("Reviewed commit contains a nonregular skill member")
        relative = path.decode("utf-8").removeprefix("skills/")
        committed[relative] = subprocess.check_output(["git", "cat-file", "blob", oid.decode("ascii")], cwd=ROOT)
    if committed != expected_files:
        raise ValueError("Current skill payload differs from the reviewed commit")
    return {name: digest(raw) for name, raw in committed.items()}
...
def main():
    ...
    common = {"date": datetime.now(timezone.utc).isoformat(), "repo": REPO, "tag": TAG,
              ...
              "reviewed_commit_payload_manifest": commit_manifest,
              "current_payload_matches_reviewed_commit": True}
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="story-v054-remote-install-") as folder:
            ...
            installed.update(ok=True, method="official installer --method download --ref v0.5.4",
        ...
        final_ref = api(f"repos/{REPO}/git/ref/tags/{TAG}")
        final_obj = final_ref["object"]
        for _ in range(5):
            if final_obj["type"] == "commit":
                break
            if final_obj["type"] != "tag":
                raise ValueError("Remote tag no longer resolves to a commit")
            final_obj = api(f"repos/{REPO}/git/tags/{final_obj['sha']}")["object"]
        if final_obj["type"] != "commit" or final_obj["sha"] != args.expected_commit:
            raise ValueError("Remote tag changed during download or installation")
        if run(release, ["git", "rev-parse", TAG + "^{commit}"]).strip() != args.expected_commit:
            raise ValueError("Local tag changed during verification")
        release.update(final_resolved_commit=final_obj["sha"], tag_rechecked_after_install=True)
        installed.update(tag_rechecked_after_install=True)

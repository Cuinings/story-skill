#!/usr/bin/env python3
"""Read-only v0.5.1 Release download and official isolated-install verification."""
...
def main():
    ...
    if len(expected) != 33 or package.current_version() != VERSION:
        raise ValueError("Current source is not the expected 33-file v0.5.1 suite")
    ...
    try:
        ...
        with tempfile.TemporaryDirectory(prefix="story-v051-remote-install-") as folder:
            ...
            installed.update(ok=True, method="official installer --method download --ref v0.5.1",

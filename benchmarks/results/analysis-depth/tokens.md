...
|---|---:|---:|---:|
| 长篇单章：明确必读指令下限 | 30,979 | 5,324 | 82.81% |
| 多线超长篇：含完整超长篇附加流程 | 30,979 | 7,091 | 77.11% |
| 短篇：上游仅入口 vs 新版入口与流程 | 10,422 | 5,324 | 48.92% |
| 长篇拆文：上游仅入口 vs 新版入口与流程 | 8,996 | 4,063 | 54.84% |
| 深读示范：含按需分析对照样例 | 8,996 | 5,600 | 37.75% |
| 审稿：上游仅入口 vs 新版入口与流程 | 11,334 | 2,710 | 76.09% |
| 冲突与看点审查：含中文场景参考 | 11,334 | 4,040 | 64.36% |

原始文件清单、SHA-256 和每个场景的统计边界见 [tokens.json](tokens.json)。

发现元数据：上游 13 个入口合计 1257 tokens；新版 7 个入口 308 tokens。仅含名称/描述；旧技能仍启用时，不能把这个差额当成真实节省。

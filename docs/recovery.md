...
以下命令均需 `python "<技能目录>/scripts/story.py"` 前缀与 `--book "<书目录>"`。技能安装更新后沿用原 `.story/state.sqlite3`，无须重新初始化；数据库 schema 保持 1。
...
更早章节、导入基线和已完成报告的外部修改不会被强制覆盖。先交付独立修订稿；若要把历史修订作为后续写作基线，按 [写作流程](../.agents/skills/story-codex/references/write.md) 在独立目录复核导入最后完整章和卡片。

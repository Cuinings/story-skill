...
以下命令均需 `python "<技能目录>/scripts/story.py"` 前缀与 `--book "<书目录>"`。新版数据库 schema 为 2。schema1 书库先停止写入，保留原书并复制到独立目录，再 `migrate --book "<副本>"`。迁移前用 SQLite backup 生成一致备份并检查完整性；所有结构变更在同一事务中进行，失败回滚。迁移备份位于 `.story/migration-backups/`。回退时停止相关进程，在另一个独立书目录恢复备份并使用 v0.2.0；备份之后的新修改不会自动出现在旧版中。无须重新初始化或修改书籍身份。
...
更早章节及含结构化世界变化的章节使用 [历史分支](../.agents/skills/story-codex/references/long-form.md)。候选、旧版、证据及审核状态分别保存；复核没有完成不能发布。导入基线和已完成分析报告仍不允许无条件覆盖。
...
已验证本机 Windows NTFS/exFAT 的相关路径；尚未完成 POSIX 实机、长篇盲评或账单 token 对比。

...
回归测试包含实际 SQLite 第二连接锁定、提交失败回滚、旧分析记录兼容，以及导出和安装各阶段的并发保存、目录重建、安装进程强退。导出使用同一文件系统的硬链接进行不覆盖发布，要求目录所在文件系统支持它；失败返回待恢复，不退回无条件覆盖。没有全局锁住外部编辑器，后续保存会成为新的外部修改，由 `status` 检出。

本次修复改善恢复与证据完整性，不代表已完成真实长篇盲评、跨文件系统验证或账单 token 对比。

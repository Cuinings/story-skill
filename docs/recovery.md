...
以下 CLI 示例均需 `python "<核心技能目录>/scripts/story.py"` 前缀；书籍命令另附 `--book "<书目录绝对路径>"`，`template` 与帮助命令不附 `--book`。本页对应 [v0.5.3](releases/v0.5.3.md)，本版实际验证与发布状态见 [发布记录](github-release.md)，平台限制见上方；共享核心为 `story-codex`；数据库继续使用 schema 2，v0.3.0、v0.4.0、v0.5.0、v0.5.1、v0.5.2 书库无需重新导入。分卷正文命名只应用到新保存的目标，已有旧章路径继续识别。
...
v0.5.2 增加历史审查级别兼容、卡片与后补世界证据的修订保护、历史候选与依赖读取，以及分析定位、版本校验和导出恢复。v0.5.3 沿用这些能力，发布状态与本版验证见 [v0.5.3 记录](../benchmarks/results/v0.5.3/README.md)，不将历史通过结果当成本版已通过。新书默认书名目录由技能选择书根后建立，既有书不会因此自动搬动；需要整理旧书时先核对现有目录和状态。[书目录规则](目录结构.md#小说一书一目录状态与可见稿件分开)

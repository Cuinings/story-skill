...
文档版本：v0.5.1，已于 2026-09-10 发布。本版沿用 7 个技能入口和具名分卷正文，完整套件为 33 个载荷文件，新增作品深读规范、按需分析样例，以及分析稿修订和断点续跑说明。[v0.5.1 版本说明](releases/v0.5.1.md) 汇总变化；固定标签、Release 附件、远端 CI 和 Packages 的实际状态逐项登记在 [发布验证记录](github-release.md)。

Windows 正文和报告导出的已知 WinError 32 尚未修复，Windows 用户继续使用固定 v0.4.0；已经用新版写入的书先完整备份，不直接覆盖或降级。本页当前用法对应 v0.5.1，旧版测量和发布记录保留各自版本范围。
...
v0.5.1 的工程验收、平台结果和分发文件以 [版本说明](releases/v0.5.1.md) 与 [发布验证记录](github-release.md) 为准。作品分析的程序核验与文学评阅分别报告，不用测试通过替代解释是否成立。

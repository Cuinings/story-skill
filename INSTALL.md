...
**平台选择：macOS／Linux 安装 v0.5.5，固定标签、Release 附件与官方隔离安装已核验。Windows 正文及报告导出存在 WinError 32，会保留待恢复状态；Windows 用户暂用已验证的 v0.4.0，不自动升级到 v0.5.5。** [现存限制与证据](docs/releases/v0.5.5.md#验证范围)
...
macOS／Linux 按下表安装或升级到 v0.5.5。先核验固定标签解析出的提交和实际下载附件，下载或摘要不符时保留原安装。Windows 保持或安装固定 v0.4.0，不自动升级至 v0.5.5。已经在 Windows 安装 0.5.x 时先保留技能备份和书籍状态，不自动降级或覆盖待恢复文件。[v0.4.0 验证记录](docs/github-release.md#历史-v040-发布验证记录)。一次安装过程中固定使用同一标签及其解析出的提交；已安装版本高于平台目标时，停止自动降级并说明情况。

| 参数 | 值 |
|---|---|
| 仓库 | `NingCui29/story-skill` |
| macOS／Linux 目标标签 | `v0.5.5` |
| 发布状态 | 已发布，固定标签、附件回下载与官方隔离安装均已核验；[本版验证目录](benchmarks/results/v0.5.5/README.md) |
| 标签提交 | `c53703bb57f42504022edb1c5d5011943e53cdf1`；[标签与发布核验](benchmarks/results/v0.5.5/release/release.json) |
| Python | 3.10 或更高 |
| 目标载荷 | 7 个同级技能目录，共 33 个文件；已核对固定提交源码、Release ZIP 与官方固定标签安装的一致性 |
| Release ZIP | [story-codex-0.5.5.zip](https://github.com/NingCui29/story-skill/releases/download/v0.5.5/story-codex-0.5.5.zip) · [校验文件](https://github.com/NingCui29/story-skill/releases/download/v0.5.5/story-codex-0.5.5.zip.sha256) |
| ZIP SHA-256 | `c3b621859256ef9cdd350bcfde1d09c959c7e4b45c423082b83b26133e1711db`；本地构建、远端回下载与 GitHub 服务端摘要一致，[发布核验](benchmarks/results/v0.5.5/release/release.json) |
...
6. **核验并报告。** 再次比对目标的七个技能及本平台全部载荷文件（macOS／Linux 33 个，Windows 31 个）与已验证 ZIP。使用新核心的 `scripts/story.py` 在隔离临时书目录执行 `--version`、`--help`、`init` 和 `status`；只将版本与本次平台目标一致（macOS／Linux 为 `0.5.5`，Windows 为 `0.4.0`）、全部文件和四个命令都通过的结果报告为成功。汇报安装位置、版本、新装／升级／无需更新、备份位置及本地修改差异。告诉用户技能可在下一条消息使用；未显示时重启 Codex。

以上操作只更新技能文件。已有 0.3.0／0.4.0 书库无需重新导入，小说正文、书籍状态和创作约定保留在原位置；旧导出不会自动批量移动。v0.5.5 补齐历史修订决定恢复、关联后文复核和世界状态衔接，调整局部点评、资料问答与细纲范围的默认行为；沿用新书书名目录、具名分卷与固定章名的正文规则；新增正文按 `chapters/第一卷 雨夜/第1章 雨中来客.md` 这样的具名分卷路径保存，实际输出路径以工具回执为准。[本版变化与兼容说明](docs/releases/v0.5.5.md)

## 官方脚本参数示例

下列参数用于 macOS／Linux 的第 2 步隔离下载；Windows 将 `--ref v0.5.5` 改为 `--ref v0.4.0`，七个路径不变。Codex 应替换本机安装器位置与新建的临时目录：

```bash
python3 "<skill-installer目录>/scripts/install-skill-from-github.py" --repo NingCui29/story-skill --ref v0.5.5 --path skills/story-codex skills/story-codex-plan skills/story-codex-write skills/story-codex-analyze skills/story-codex-review skills/story-codex-research skills/story-codex-cover --dest "<隔离临时目录>/skills"
```

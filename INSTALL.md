...
本页当前安装目标为已发布的 **v0.4.0**。维护者在新稳定版发布并验证后更新本页的目标；一次安装过程中固定使用同一标签及其解析出的提交。用户指定版本时，以该版本的实际布局为准。已安装版本明确高于目标时，停止自动降级并说明情况。

| 参数 | 值 |
|---|---|
| 仓库 | `NingCui29/story-skill` |
| 当前标签 | `v0.4.0` |
| 标签提交 | `c1b3c3377573610a191e165ceb6866ae35afe5a8` |
| Python | 3.10 或更高 |
| 目标载荷 | 7 个同级技能目录，共 31 个文件 |
| [Release ZIP](https://github.com/NingCui29/story-skill/releases/download/v0.4.0/story-codex-0.4.0.zip) 的 SHA-256 | `087ad76fe32714ea776853cab579c3b6087ed092ef0857c55a76ae20aff7d54b` |
...
2. **先准备完整新版。** 找到并读取本机 `skill-installer`，使用它的官方脚本，先安装到新建的隔离临时目录。展开全部七个 `--path`，固定 `--ref v0.4.0`；`--dest` 是临时 skills 父目录。下载本版 Release ZIP 及其 [校验文件](https://github.com/NingCui29/story-skill/releases/download/v0.4.0/story-codex-0.4.0.zip.sha256)，核对上表的 SHA-256，再逐文件比较临时安装与 ZIP。ZIP 中的成员必须是这七个目录内的普通文件，拒绝绝对路径、路径越界及链接。核对七份 `SKILL.md`、共享运行时及全部 31 个文件后再操作目标。下载或验证失败时，保留原安装。
...
6. **核验并报告。** 再次比对目标的七个技能及 31 个载荷文件与已验证 ZIP。使用新核心的 `scripts/story.py` 在隔离临时书目录执行 `--version`、`--help`、`init` 和 `status`；只将版本 `0.4.0`、全部文件和四个命令都通过的结果报告为成功。汇报安装位置、版本、新装／升级／无需更新、备份位置及本地修改差异。告诉用户技能可在下一条消息使用；未显示时重启 Codex。

以上操作只更新技能文件。已有 0.3.0 书库无需重新导入，小说正文、书籍状态和创作约定保留在原位置。

## 官方脚本参数示例

下列参数用于第 2 步的隔离下载；Codex 应替换本机安装器位置与新建的临时目录：

```powershell
python "<skill-installer目录>/scripts/install-skill-from-github.py" --repo NingCui29/story-skill --ref v0.4.0 --path skills/story-codex skills/story-codex-plan skills/story-codex-write skills/story-codex-analyze skills/story-codex-review skills/story-codex-research skills/story-codex-cover --dest "<隔离临时目录>/skills"
```
